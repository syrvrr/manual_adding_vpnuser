...
if [ -z "$VPN_USER" ] || [ -z "$VPN_PASSWORD" ] || [ -z "$IP_ADDR" ]; then
cat 1>&2 <<EOF
Usage: sudo sh $0 'username_to_add' 'password_to_add' 'ip_addr'
EOF
  exit 1
fi

if printf '%s' "$VPN_USER $VPN_PASSWORD $IP_ADDR " | LC_ALL=C grep -q '[^ -~]\+'; then
  exiterr "VPN credentials must not contain non-ASCII characters."
fi

case "$VPN_USER $VPN_PASSWORD $IP_ADDR" in
  *[\\\"\']*)
...
"$VPN_USER" l2tpd "$VPN_PASSWORD" $IP_ADDR
EOF
